...
class DeviceControllerWebMvcTest {
    ...
    class Validation {
        ...
        void createBlankFieldsReturns400() throws Exception {
            ...
            mockMvc.perform(post(DEVICES_PATH)
                    ...
                    .andExpect(jsonPath("$.errors.deviceTypeId").exists())
                    .andExpect(jsonPath("$.errors.serialNumber").exists())
                    .andExpect(jsonPath("$.errors.installationDate").exists());
        }
    ...
    class ExceptionMapping {
        ...
        void getByIdNotFoundReturns404() throws Exception {
            ...
            mockMvc.perform(get(DEVICES_PATH + "/{id}", DEVICE_ID_NOT_FOUND)
                    ...
                    .andExpect(jsonPath("$.detail").value("Device not found with id: " + DEVICE_ID_NOT_FOUND))
        ...
        void deleteNotFoundReturns404() throws Exception {
            doThrow(new EntityNotFoundException("Device not found with ID: " + DEVICE_ID_NOT_FOUND))
    ...
    class ResponseShape {
        ...
        @Test
        @DisplayName("GET by id exposes mapped foreign-key ids (deviceTypeId, organizationId)")
        void getByIdExposesMappedIds() throws Exception {
            when(deviceService.getDeviceById(DEVICE_ID)).thenReturn(device);
        ...
        @Test
        @DisplayName("GET by id exposes departmentId when device belongs to a department")
        void getByIdExposesDepartmentId() throws Exception {
            Department department = Department.builder().id(DEPARTMENT_ID).build();
            Device deviceInDepartment = Device.builder()
                    .id(DEVICE_ID)
                    .name(DEVICE_NAME)
                    .deviceType(DeviceType.builder().id(DEVICE_TYPE_ID).build())
                    .serialNumber(DEVICE_SERIAL_NUMBER)
                    .installationDate(INSTALLATION_DATE)
        ...
        void getDevicesReturnsPagedJson() throws Exception {
        ...
        @Test
        @DisplayName("GET (list) returns 200 with an empty content array when there are no devices")
        void getDevicesReturnsEmptyPagedJson() throws Exception {
            Pageable pageable = PageRequest.of(0, 20);
